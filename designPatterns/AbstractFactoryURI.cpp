...
// Define the interface for the ElectricalDeviceFactory class
class IElectricalDeviceFactory
{
public:
  virtual std::shared_ptr<IElectricalDevice> createElectricalDevice(double voltage, double current) = 0;
};

// Implement the ElectricalDeviceFactory class
class ElectricalDeviceFactory : public IElectricalDeviceFactory
{
public:
  std::shared_ptr<IElectricalDevice> createElectricalDevice(double voltage, double current) override
  {
    return std::make_shared<ElectricalDevice>(voltage, current);
  }
};

// Define the interface for the OhmsLawCalculatorFactory class
class IOhmsLawCalculatorFactory
{
public:
  virtual std::unique_ptr<IOhmsLawCalculator> createOhmsLawCalculator() = 0;
};

// Implement the OhmsLawCalculatorFactory class
class OhmsLawCalculatorFactory : public IOhmsLawCalculatorFactory
{
public:
  std::unique_ptr<IOhmsLawCalculator> createOhmsLawCalculator() override
  {
    return std::make_unique<OhmsLawCalculator>();
  }
};
...
int main(int argc, char* argv[])
{
  ...
  // Create an instance of the ElectricalDeviceFactory and use it to create an ElectricalDevice object
  std::unique_ptr<IElectricalDeviceFactory> deviceFactory = std::make_unique<ElectricalDeviceFactory>();
  std::shared_ptr<IElectricalDevice> device = deviceFactory->createElectricalDevice(voltage, current);

  // Create an instance of the OhmsLawCalculatorFactory and use it to create an OhmsLawCalculator object
  std::unique_ptr<IOhmsLawCalculatorFactory> calculatorFactory = std::make_unique<OhmsLawCalculatorFactory>();
  std::unique_ptr<IOhmsLawCalculator> calculator = calculatorFactory->createOhmsLawCalculator();

  // Create an adapter that adapts the ElectricalDevice to the IOhmsLawCalculator interface
  std::unique_ptr<IOhmsLawCalculator> adapter = std::move(calculator);

  // Use the adapter to calculate the resistance of the device
  double resistance = adapter->calculateResistance(device , device->getCurrent());
